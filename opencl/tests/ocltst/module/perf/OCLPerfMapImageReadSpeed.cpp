...
static const unsigned int Iterations[2] = {1, OCLPerfMapImageReadSpeed::NUM_ITER};

OCLPerfMapImageReadSpeed::OCLPerfMapImageReadSpeed() { _numSubTests = NUM_SIZES * NUM_FORMATS * 2; }
...
void OCLPerfMapImageReadSpeed::open(unsigned int test, char* units, double& conversion,
...
void OCLPerfMapImageReadSpeed::run(void) {
  ...
  // Warm up
  mem = _wrapper->clEnqueueMapImage(cmd_queue_, outBuffer_, CL_TRUE, CL_MAP_READ, origin, region,
