...
bool
Flag::setValue(const char* value)
{
    if (value_ == NULL) {
...
#define DEFINE_RELEASE_FLAG_STRUCT(type, name, value, help) \
{ #name, &name, T##type, true },
...
{
    RUNTIME_FLAGS( \
        DEFINE_DEBUG_FLAG_STRUCT, \
        DEFINE_RELEASE_FLAG_STRUCT, \
        DEFINE_DEBUG_FLAG_STRUCT )
...
#define DEFINE_DEBUG_FLAG_VALUE(type, name, value, help) \
    DEBUG_ONLY(type name = value);

RUNTIME_FLAGS( \
    DEFINE_DEBUG_FLAG_VALUE, \
    DEFINE_RELEASE_FLAG_VALUE, \
    DEFINE_DEBUG_FLAG_VALUE );
