...
// All of host conversion is from fcbx - Float Conversion Blaster XTREME.
namespace fcbx {
...
__OCP_FP_HOST_DEVICE_STATIC__ T to_float(uint32_t u32, int8_t scale_exp) {
  ...
  if (enc.MxScale) {
    ...
    if (tmp < dstEnc.MinExp) {
      ...
      // Overflow: return infinity (MI350 HW behavior)
...
__OCP_FP_HOST_DEVICE_STATIC__ uint32_t from_float_sr(T f, uint32_t seed, int8_t scale_exp) {
  ...
  if (enc.MxScale) {
    // Must adjust to 1.xxx form according to MI350_SP_Changes 6.2.1
